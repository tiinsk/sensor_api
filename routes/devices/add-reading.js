...
module.exports = {
  ...
  handler: (request, h) => {
    if(request.auth.credentials.device !== request.params.id) {
      return Boom.conflict(`Device can add records to only itself (JWT device (${request.auth.credentials.device}) does not match parameter id (${request.params.id}))`);
    }
    ...
    return knex.transaction( async trx => {
      ...
      const result = await trx('reading').insert({
